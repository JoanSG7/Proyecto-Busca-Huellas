...
Dentro de `Documentacion/` hay varios scripts `migracion_*.sql`. Son actualizaciones que se aplicaron sobre el esquema inicial. Si ejecutaste `busca_huellas.sql` reciente **no son necesarios**, pero si estás actualizando una instalación anterior revisa cada uno antes de aplicarlo.

Las imágenes de artículos no necesitan una tabla adicional: se almacenan como archivos en `static/uploads/articulos/` y la tabla `articulo` conserva su ruta en `url_imagen` (y en el contenido cuando son imágenes intermedias). Por eso no se debe ejecutar `migracion_imagenes_articulos_en_bd.sql`.
