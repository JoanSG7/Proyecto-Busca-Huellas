...
def _normalizar_url_imagen_articulo(url_imagen):
    """Convierte rutas antiguas de la BD en URLs que el navegador puede abrir.

    Las imágenes nuevas se guardan como ``/static/uploads/...``, pero algunos
    registros existentes pueden tener ``static/uploads/...``, ``uploads/...``
    o una ruta local de Windows. Esas rutas relativas se resolvían contra la
    página actual y terminaban en un 404.
    """
    ruta = str(url_imagen or "").strip()
    if not ruta:
        return None

    # Las URLs remotas y data URLs ya son utilizables por el navegador.
    if ruta.startswith(("http://", "https://", "//", "data:")):
        return ruta

    ruta = ruta.replace("\\", "/")
    marcador_static = "/static/"
    posicion_static = ruta.lower().find(marcador_static)
    if posicion_static >= 0:
        return ruta[posicion_static:]
    if ruta.lower().startswith("static/"):
        return f"/{ruta}"
    if ruta.startswith("/"):
        return ruta

    ruta = ruta.lstrip("/")
    if ruta.lower().startswith("uploads/"):
        return f"/static/{ruta}"
    if ruta.lower().startswith("articulos/"):
        return f"/static/uploads/{ruta}"

    # Los nombres generados por la aplicación se almacenan dentro de artículos.
    if "/" not in ruta:
        return f"/static/uploads/articulos/{ruta}"
    return ruta
    ...
    for bloque in bloques:
        ...
        if valor:
            if bloque["tipo"] == "imagen":
                valor = _normalizar_url_imagen_articulo(valor)
...
def _preparar_articulo_para_vista(articulo):
    ...
    articulo["url_imagen"] = _normalizar_url_imagen_articulo(articulo.get("url_imagen"))
    return articulo
...
def _contenido_articulo_desde_formulario():
    ...
    for indice, tipo in enumerate(tipos):
        ...
        if tipo not in ARTICLE_BLOCK_TYPES:
            continue
        if tipo == "imagen":
            imagen = request.files.get(f"bloque_imagen_{indice}")
            if imagen and imagen.filename:
                marcador = f"__BH_FILE_IMAGE_{indice}__"
                imagenes_nuevas.append((marcador, imagen))
                valor = marcador
            else:
                valor = clean_text(request.form.get(f"bloque_imagen_actual_{indice}"), 255)
        if valor:
            bloques.append({"tipo": tipo, "valor": valor})

    if not bloques:
        return contenido_plano, len(contenido_plano), imagenes_nuevas
    ...
    return contenido, texto_total, imagenes_nuevas


def _guardar_imagenes_de_bloques(contenido, imagenes_nuevas):
    """Guarda las imágenes del editor en static/uploads/articulos.

    La base de datos conserva únicamente las rutas, igual que el respaldo SQL
    principal. Esto evita depender de una tabla adicional de BLOBs.
    """
    for marcador, imagen in imagenes_nuevas:
        ruta = guardar_imagen(imagen, "articulos")
        contenido = contenido.replace(marcador, ruta or "")
    return contenido


def _guardar_imagen_principal_articulo(imagen):
    """Devuelve la ruta pública de la portada o None si no se adjuntó una válida."""
    return guardar_imagen(imagen, "articulos")
...
def mostrar_inicio():
    articulos = [_preparar_articulo_para_vista(articulo) for articulo in listar_articulos()[:3]]
...
def _es_envio_asincrono_chat():
    return request.headers.get("X-Requested-With") == "XMLHttpRequest"


def _preparar_ubicaciones_chat(mensajes):
    """Crea enlaces directos a las coordenadas guardadas en cada avistamiento.

    Los avistamientos existentes almacenan ``dirección · latitud, longitud``
    en una sola columna. Conservamos ese formato compatible y extraemos las
    coordenadas para que Google Maps abra el punto exacto, no una búsqueda
    aproximada por texto.
    """
    patron_coordenadas = re.compile(r"(-?\d{1,2}\.\d+)\s*,\s*(-?\d{1,3}\.\d+)")
    mensajes_preparados = []
    for mensaje in mensajes:
        mensaje = dict(mensaje)
        ubicacion = (mensaje.get("ubicacion_avistamiento") or "").strip()
        coincidencia = patron_coordenadas.search(ubicacion)
        if coincidencia:
            coordenadas = f"{coincidencia.group(1)},{coincidencia.group(2)}"
            mensaje["ubicacion_mapa_url"] = (
                "https://www.google.com/maps/dir/?api=1&"
                + urlencode({"destination": coordenadas})
            )
            mensaje["ubicacion_etiqueta"] = ubicacion[:coincidencia.start()].rstrip(" ·|- ") or ubicacion
        elif ubicacion:
            mensaje["ubicacion_mapa_url"] = (
                "https://www.google.com/maps/search/?api=1&"
                + urlencode({"query": ubicacion})
            )
            mensaje["ubicacion_etiqueta"] = ubicacion
        mensajes_preparados.append(mensaje)
...
def mostrar_chat_alerta(id_alerta):
    ...
    if request.method == "POST":
        ...
        if len(mensaje) < 2:
            texto_error = "Escribe un mensaje antes de enviarlo."
        elif _mensaje_tiene_palabras_bloqueadas(mensaje):
            texto_error = "Tu mensaje contiene palabras fuertes o indebidas. Reescríbelo con respeto."
        elif not receptor:
            texto_error = "No hay un usuario receptor para este chat."
        else:
            crear_mensaje_alerta(id_alerta, current_user_id(), receptor, mensaje)
            if _es_envio_asincrono_chat():
                return jsonify({
                    "ok": True,
                    "mensaje": mensaje,
                    "autor": session.get("usuario_nombre") or "Tú",
                    "fecha": datetime.now().strftime("%Y-%m-%d %H:%M"),
                }), 201
            return redirect(url_for("mensaje.chat_alerta", id_alerta=id_alerta))

        if _es_envio_asincrono_chat():
            return jsonify({"ok": False, "error": texto_error}), 400
        flash(texto_error, "error")

    marcar_mensajes_como_leidos(id_alerta, current_user_id())
    mensajes = _preparar_ubicaciones_chat(
        listar_mensajes_alerta(chat["id_usuario_alerta"], chat["id_dueno"])
    )
    return render_template(
...
def registrar_articulo():
    ...
    contenido, contenido_texto_total, imagenes_bloques = _contenido_articulo_desde_formulario()
    imagen_principal = request.files.get("imagen_articulo")

    if len(titulo) < 5 or contenido_texto_total < 20:
        flash("El artículo necesita un título y contenido más completos.", "error")
        return redirect(url_for("articulo.lista_articulos"))

    id_articulo = crear_articulo(current_user_id(), titulo, contenido)
    contenido = _guardar_imagenes_de_bloques(contenido, imagenes_bloques)
    url_imagen = _guardar_imagen_principal_articulo(imagen_principal)
...
def editar_articulo(id_articulo):
    ...
    contenido = _guardar_imagenes_de_bloques(contenido, imagenes_bloques)
    url_imagen = _guardar_imagen_principal_articulo(imagen_principal) or articulo.get("url_imagen")
    actualizar_articulo(id_articulo, titulo, contenido, url_imagen)
